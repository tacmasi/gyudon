...
echo "N = 1"
#第1期データをコピー(カナは半角to全角, スペースは全角to半角を噛ませる -> 'FC' 削除)
./kana_han2zen.gawk ./data/detail$(head -1 date.csv).csv| sed 's/　/ /g' |sed 's/FC//g'> ./data/detail.csv
##
...
#todaydate分b.tmpへ(カナは半角to全角, スペースは全角to半角を噛ませる-> 接頭'FC'削除)
./kana_han2zen.gawk ./data/detail$todaydate.csv |sed 's/　/ /g'|sed 's/FC//g'|sort -k1,1 -t, >./data/b.tmp
